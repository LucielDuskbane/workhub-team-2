...
func SetupRoutes(r *gin.Engine) {
	...
	api := r.Group("/api/v1")
	{
		...
		company.Use(middleware.AuthMiddleware(), middleware.RoleMiddleware("employer"))
		{
			company.POST("", companyHandler.CreateCompany)
			company.GET("/me", companyHandler.GetMyCompany)
		...
		jobs := api.Group("/jobs")
		{
			...
			jobs.POST(
				"/:id/apply",
				middleware.AuthMiddleware(),
				middleware.RoleMiddleware("jobseeker"),
			...
			jobs.GET(
				...
				middleware.AuthMiddleware(),
				middleware.RoleMiddleware("employer"),
				applicationHandler.GetJobApplications,
			)
		}
		...
		admin.Use(middleware.AuthMiddleware(), middleware.RoleMiddleware("admin"))
		{
			...
			admin.GET("/companies/pending", companyHandler.GetPendingCompanies)
			admin.PATCH("/companies/:id/approve", companyHandler.ApproveCompany)
			admin.PATCH("/companies/:id/reject", companyHandler.RejectCompany)
			admin.GET("/reports", reportHandler.GetDashboardReport)
		}
	}
}
