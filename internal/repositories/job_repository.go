...
func (r *JobRepository) FindByCompanyID(companyID uint) ([]models.Job, error) {
	var jobs []models.Job
	err := config.DB.Where("company_id = ?", companyID).Find(&jobs).Error
	return jobs, err
}
...
func (r *JobRepository) Delete(id uint) error {
	return config.DB.Delete(&models.Job{}, id).Error
}
